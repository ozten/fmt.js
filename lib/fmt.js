...
exports.format = format = function (ast) {
  ...
  case 'VariableDeclaration':
    ...
    return util.format('var %s;', vars.join(',\n'));
  ...
  // for (var i=0; i < 3; i++) {...}
  // VariableDeclaration puts in the ;
  case 'ForStatement':
    return util.format('for(%s %s; %s)%s',
		       format(ast.init),
		       format(ast.test),
		       format(ast.update),
		       format(ast.body));
  ...
  case 'UpdateExpression':
      if (ast.prefix) {
	  return util.format('%s%s', ast.operator, format(ast.argument));
      } else {
	  return util.format('%s%s', format(ast.argument), ast.operator);
      }
